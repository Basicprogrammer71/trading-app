...
with tabs[0]:
    if not trades_df.empty:
        ...
        def _card(col, title, trades, pl, last):
            arrow = "&#9650;" if pl >= 0 else "&#9660;"
            color = "#00FF00" if pl >= 0 else "#FF3333"
            html = f"""<div style='background-color:#1e1e1e;padding:15px;border-radius:10px;text-align:center;color:white;'>
                <h4>{title}</h4>
        ...
        _card(c1, "📅 This Month", len(month_df), monthly_pl, last_month)
        _card(c2, "💼 Overall", len(trades_df), overall_pl, last_overall)
        _card(c3, "📆 This Year", len(year_df), yearly_pl, last_year)
...
with tabs[2]:
    ...
    if not trades_df.empty:
        ...
        for i, m in enumerate(range(1,13)):
            ...
            plm = dfm["P/L"].sum()
            bg = dark_green if plm >=0 else dark_red
            html = f"""<div style='background-color:{bg};padding:15px;border-radius:12px;text-align:center;color:white;'>
                <h5>{name}</h5>
                <p>Trades: {len(dfm)}</p>
                <p>P/L: ${plm:,.2f}</p>
            </div>"""
