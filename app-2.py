...
# Tabs
tabs = st.tabs(["Dashboard", "All Trades Table", "Historical Overview"])
...
with tabs[0]:
    if not trades_df.empty:
        ...
        with col1:
            ...
            st.write(f"P/L: ${month_df['P/L'].sum():,.2f}")
        ...
        with col2:
            ...
            st.markdown(
                ...
                f"<p>P/L: ${trades_df['P/L'].sum():,.2f}</p>"
                f"<p>Last Value: ${trades_df['Account Value'].iloc[-1]:,.2f}</p></div>",
                unsafe_allow_html=True
            )
        with col3:
            st.markdown("### 📆 This Year")
            st.write(f"Trades: {len(year_df)}")
            st.write(f"P/L: ${year_df['P/L'].sum():,.2f}")
    ...
    if not trades_df.empty:
        ...
        fig = px.line(daily_df, x="Date", y="Account Value", markers=True, title="Account Value Progress")
        fig.update_layout(xaxis_title="Date", yaxis_title="Account Value", title_x=0.5,
...
with tabs[1]:
    ...
    if not trades_df.empty:
        ...
        edited_df = st.data_editor(trades_df, num_rows="dynamic", use_container_width=True, key="trades_editor")
...
with tabs[2]:
    ...
    if not trades_df.empty:
        ...
        for i, month in enumerate(range(1,13)):
            ...
            month_pl = month_data["P/L"].sum()
            bg_color = dark_green if month_pl >= 0 else dark_red
            col = month_cols[i%4]
            with col:
                # Use HTML details tag for clickable expansion
                html = month_data.to_html(index=False)
                st.markdown(f"""<details>
<summary>
<div style='background-color:{bg_color}; padding:12px; border-radius:8px; text-align:center; color:white;'>
<strong>{month_name}</strong><br>
Trades: {len(month_data)} | P/L: ${month_pl:,.2f}
</div>
</summary>
