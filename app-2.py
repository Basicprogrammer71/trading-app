...
with tabs[2]:
    if not trades_df.empty:
        ...
        for month in range(1, 13):
            ...
            bg_color = "#d1ffd1" if month_pl >= 0 else "#ffd1d1"

            with month_cols[(month - 1) % 4]:
                st.markdown(
                    f"<div style='background-color: {bg_color}; padding: 15px; border-radius: 12px; text-align: center;'>"
                    f"<h5>{month_name}</h5>"
                    f"<p>Trades: {len(month_data)}</p>"
                    f"<p>P/L: ${month_pl:,.2f}</p>"
                    f"</div>",
                    unsafe_allow_html=True
                )
    else:
