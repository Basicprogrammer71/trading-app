...
# Initialize session state flag to control expander
if "trade_added" not in st.session_state:
    st.session_state.trade_added = False

# Expander open only if no trade was just added
...
if page == "Dashboard":
    ...
    if not trades_df.empty:
        ...
        ax.plot(trades_df["Date"], trades_df["Account Value"], marker="o", color="cyan")
